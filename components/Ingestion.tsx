...
const Ingestion: React.FC<IngestionProps> = ({ token, onNoteAdded, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus(`Synthesizing intelligence...`);
    ...
    try {
      ...
      formData.append('title', title || metadata.summary.substring(0, 40));
      formData.append('content', content);
      if (attachment) {
        formData.append('file', attachment, fileAttachment?.name || 'voice.webm');
      }
      ...
      if (response.ok) {
        ...
        // In local state, we can keep the base64 for immediate visual preview
        if (attachment) {
          const previewUrl = await toDataURL(attachment);
          newNoteFromServer.original_file.url = previewUrl;
        }
        onNoteAdded(newNoteFromServer);
        setStatus('Committed to Memory successfully.');
        setTimeout(onSuccess, 800);
      } else {
        setStatus('Failed to upload memory to cloud.');
      }
    } catch (error) {
      console.error(error);
      setStatus('Memory synthesis failed.');
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-bottom-6 duration-500">
      <header>
        <h2 className="text-3xl font-black text-slate-900 tracking-tight">Ingest Knowledge</h2>
        <p className="text-slate-500 font-medium">Capture multimodal sources. The AI will extract semantic meaning for long-term memory.</p>
      </header>
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-2">
            <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Entry Subject</label>
            <input 
              type="text" 
              value={title}
          ...
          <div className="space-y-2">
            ...
            <textarea 
              ...
              placeholder={mode === 'url' ? "https://..." : "Provide context to help the AI link this to your existing knowledge base..."}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <button type="button" onClick={() => resetMode('text')} className={`flex flex-col items-center justify-center p-5 rounded-[2rem] border-2 transition-all ${mode === 'text' ? 'bg-indigo-50 border-indigo-500 shadow-md shadow-indigo-100' : 'bg-white border-slate-100 border-dashed hover:border-slate-300'}`}>
            ...
            <button type="button" onClick={() => pdfInputRef.current?.click()} className={`flex flex-col items-center justify-center p-5 rounded-[2rem] border-2 transition-all ${mode === 'pdf' ? 'bg-indigo-50 border-indigo-500 shadow-md shadow-indigo-100' : 'bg-white border-slate-100 border-dashed hover:border-slate-300'}`}>
            ...
            <button type="button" onClick={() => resetMode('url')} className={`flex flex-col items-center justify-center p-5 rounded-[2rem] border-2 transition-all ${mode === 'url' ? 'bg-indigo-50 border-indigo-500 shadow-md shadow-indigo-100' : 'bg-white border-slate-100 border-dashed hover:border-slate-300'}`}>
            ...
            <button type="button" onClick={toggleRecording} className={`flex flex-col items-center justify-center p-5 rounded-[2rem] border-2 transition-all ${isRecording ? 'bg-red-50 border-red-500 animate-pulse shadow-md shadow-red-100' : mode === 'voice' ? 'bg-indigo-50 border-indigo-500 shadow-md shadow-indigo-100' : 'bg-white border-slate-100 border-dashed hover:border-slate-300'}`}>
            ...
            <button type="button" onClick={() => imageInputRef.current?.click()} className={`flex flex-col items-center justify-center p-5 rounded-[2rem] border-2 transition-all ${mode === 'image' ? 'bg-indigo-50 border-indigo-500 shadow-md shadow-indigo-100' : 'bg-white border-slate-100 border-dashed hover:border-slate-300'}`}>
          ...
          {status && (
            <div className="flex items-center gap-3 text-indigo-600 bg-indigo-50/50 border border-indigo-100 p-4 rounded-2xl animate-in fade-in slide-in-from-top-2">
          ...
          <div className="pt-4 flex items-center justify-end border-t border-slate-100">
            <button 
              ...
              className="bg-indigo-600 text-white px-12 py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-700 shadow-2xl shadow-indigo-100 transition-all disabled:opacity-50 active:scale-95"
            >
              Commit to Memory
            </button>
          </div>
